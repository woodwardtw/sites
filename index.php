...
function just_update_tags($post_id){
    $api = get_post_meta($post_id, 'full-api-url', true);
    $response = wp_remote_get( $api,  array( 'timeout' => 25 ) );
...
//SCHEDULE THE tag update TO RUN EVERY 24HRS VIA WP CRON

if( !wp_next_scheduled( 'tag_updates' ) ) {
   wp_schedule_event( time(), 'daily', 'tag_updates' );
}

add_action( 'tag_updates', 'run_import' );